...
import static com.hs.monitor.message.MessageHandler.handleMsg;
import static com.hs.monitor.protocol.ProtocolUtil.PROTOCOL_RESOLVE;
...
public class DeviceEmulator extends Application {
    ...
    private static SerialPort[] ports = Serial.getPortArr();
    private static SerialPort serialPort = Arrays.stream(ports)
            .filter(port -> port.getSystemPortName().equals("COM2"))
            .findFirst()
            .orElse(null);
    ...
    static {
        ...
        if (serialPort.openPort()) {
            System.out.println(serialPort.getSystemPortName() + "打开成功");
