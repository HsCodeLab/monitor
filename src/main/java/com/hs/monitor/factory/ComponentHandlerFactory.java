...
import com.hs.monitor.maps.PageMap;
import com.hs.monitor.utils.LanUtil;
...
import static com.hs.monitor.service.HistoryService.getTableTotal;
import static com.hs.monitor.utils.LanUtil.*;
import static com.hs.monitor.utils.TableUtil.createPage;

public class ComponentHandlerFactory {
    private static final Map<String, Consumer<ControlWrapper>> handlerMap = new HashMap<>();

    /**
     * 1.如果是像1-247(用减号-分隔)这种形式,就说明是从数字1到247,
     * 2.逗号分隔
     * items配置中会有类似"items=yyyy-MM-dd"的格式
     *
     * @param items
     * @return
     */
    public static String[] getItems(String items) {
        ...
        if (items.contains("-") && !items.contains(",")) {
            ...
            int start = Integer.parseInt(itemArr[0]);
            int end = Integer.parseInt(itemArr[1]);
...
//                String[] arr = items.split(",");
                ...
                LAN.addListener((observable, oldValue, newValue) -> {
                    String[] arr2 = new String[finalArr.length];
                    for (int i = 0; i < arr1.length; i++) {
                        arr2[i] = getText(finalArr[i]);
                    }
        ...
        handlerMap.put("CheckComboBox", (wrapper) -> {
            ...
            String all = getText("modbus.all");
//            String none = getText("modbus.none");
//            boolean isAll = true;//是否为全选

            checkComboBox.getItems().add(all);

            checkComboBox.getItems().addAll(FXCollections.observableArrayList(arr));
//            checkComboBox.getCheckModel().checkIndices(1, 3);
...
//            handleCheckComboBox(checkComboBox);
        ...
        handlerMap.put("Spinner", (wrapper) -> {
            ...
            if (wrapper.getValueProperty() != null) {
                SpinnerValueFactory<Double> valueFactory = new SpinnerValueFactory.DoubleSpinnerValueFactory(Double.parseDouble(wrapper.getMap().get("min")), Double.parseDouble(wrapper.getMap().get("max")), Double.parseDouble(wrapper.getMap().get("value")), Double.parseDouble(wrapper.getMap().get("step")));
                spinner.setValueFactory(valueFactory);
                Optional.ofNullable(wrapper.getValueProperty()).ifPresent(valueProperty -> valueProperty.addListener((observable, oldValue, newValue) -> {
                    Double value;
                    try {
...
//                ));
        ...
        handlerMap.put("ListView", (wrapper) -> {

            ListView listView = (ListView) wrapper.getControl();
            listView.prefHeightProperty().set(200);
    ...
    public static String package_prefix = "com.hs.monitor.handlers.";
...
//                                    System.out.println("单值中调用时的wrapper=" + wrapper);
                                    method.invoke(null, wrapper.getControl());
                                    break OUTER;
                                }
                            }
