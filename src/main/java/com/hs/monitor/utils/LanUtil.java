...
//                log.info("找不到" + keyName + "对应的文本");
    ...
    public static void addLanguageListen(String keyName, Object control) {
        if (control instanceof Labeled) {
            ...
            CheckComboBox<String> checkComboBox = (CheckComboBox<String>) control;
            LAN.addListener((observable, oldValue, newValue) -> {
                ...
                itemsArray[0] = getText(keyName);
                checkComboBox.getItems().clear();
                checkComboBox.getItems().addAll(itemsArray);
            });
        } else if (control instanceof Stage) {
            Stage stage = (Stage) control;
            LAN.addListener((observable, oldValue, newValue) -> stage.setTitle(getText(keyName)));
        }
    }
