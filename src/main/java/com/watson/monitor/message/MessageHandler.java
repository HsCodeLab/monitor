...
import com.watson.monitor.protocol.ProtocolByte;
import com.watson.monitor.protocol.ProtocolStr;
...
public class MessageHandler {
    ...
    private static void handleConsumer00(byte[] bytes, String str) {
        WConsumer consumer = ProtocolByte.function_map.get(bytes[2]);
        accept(consumer);
    }

    private static void handleConsumer01(byte[] bytes, String str) {
        WConsumer consumer = ProtocolStr.function_map.get(str);
        accept(consumer);
    }

    private static void accept(WConsumer consumer){
        if (consumer != null) {
            consumer.accept();
        } else {
